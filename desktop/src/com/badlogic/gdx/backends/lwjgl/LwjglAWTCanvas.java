 ...
 * Overriden by tsagrista to add MSAA to the PixelFormat.
 * @author Nathan Sweet */
public class LwjglAWTCanvas implements Application {
	...
	this(listener, null, config);
    }

    public LwjglAWTCanvas(ApplicationListener listener, LwjglAWTCanvas sharedContextCanvas, LwjglApplicationConfiguration config) {
	LwjglNativesLoader.load();

	AWTGLCanvas sharedDrawable = sharedContextCanvas != null ? sharedContextCanvas.canvas : null;
	try {
	    canvas = new AWTGLCanvas(GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice(), new PixelFormat(0, 8, config.stencil, config.samples),
		    sharedDrawable) {
