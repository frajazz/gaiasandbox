...
public class CollapsiblePane extends VerticalGroup {
	 ...
	 *            expand/detach icons.
	 */
	public CollapsiblePane(final Stage stage, final String labelText,
			final Actor content, final Skin skin, String labelStyle,
			String expandButtonStyle, String detachButtonStyle,
			Actor... topIcons) {
		super();
		...
		expandIcon.addListener(new TextTooltip(I18n.bundle
				.get("gui.tooltip.expandcollapse.group"), skin));
		...
		detachIcon.addListener(new EventListener() {
			...
			public boolean handle(Event event) {
				if (event instanceof ChangeEvent) {
					dialogWindow = createWindow(labelText, content, skin,
							stage, lastx, lasty);
		...
		detachIcon.addListener(new TextTooltip(I18n.bundle
				.get("gui.tooltip.detach.group"), skin));
	 ...
	 *            expand/detach icons.
	 */
	public CollapsiblePane(Stage stage, String labelText, final Actor content,
			Skin skin, Actor... topIcons) {
		this(stage, labelText, content, skin, "header", "expand-collapse",
				"detach", topIcons);
	}

	private CollapsibleWindow createWindow(String labelText,
			final Actor content, Skin skin, Stage stage, float x, float y) {
		...
		/** Close button **/
		TextButton close = new OwnTextButton(I18n.bundle.get("gui.close"),
				skin, "default");
		close.setName("close");
