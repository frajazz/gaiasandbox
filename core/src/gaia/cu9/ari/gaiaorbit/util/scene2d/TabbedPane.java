...
public class TabbedPane extends Table {
	 ...
	 *            the skin
	 * @param tabTitleAlign
	 *            the alignment for tab titles. Must be one of
	 *            {@link Align#left}, {@link Align#center} or
	 *            {@link Align#right}.
	 */
	...
	/**
	 * Creates a {@code TabbedPane} using the specified skin, style name and
	 * alignment.
	 * 
	 * @param skin
	 *            the skin
	...
	/**
	 * Creates a {@code TabbedPane} without setting the style or size. At least
	 * a style must be set before using this tabbed pane.
	 */
	public TabbedPane() {
		initialize();
	}
	...
	private void initialize() {
		...
		default:
			throw new IllegalArgumentException(
					"TabbedPane align must be one of left, center, right");
		}
	...
	/**
	 * Returns the tabbed pane's style. Modifying the returned style may not
	 * have an effect until {@link #setStyle(TabbedPaneStyle)} is called.
	 */
	...
	public void addTab(String title, Actor actor) {
		...
		button.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				TabTitleButton tabTitleButton = (TabTitleButton) event
						.getListenerActor();
				// if (tabTitleButton.isChecked())
				setSelectedIndex(tabTitleButton.index);
			}
		});
		tabTitleTable.add(button); // .uniform().fill(); // uniform gives tabs
									// the same size
	...
	private void setSelectedTab(boolean value) {
		TabTitleButton tabTitleButton = ((TabTitleButton) tabTitleTable
				.getCells().get(selectedIndex).getActor());
	...
	/**
	 * Sends a ChangeEvent, with this TabbedPane as the target, to each
	 * registered listener. This method is called each time there is a change to
	 * the selected index.
	 */
	...
	private static class TabTitleButton extends TextButton {
		...
		// }
		//
		// private TabTitleButton(int index, String text, Skin skin, String
		// styleName) {
		...
		// }
		//
		// private TabTitleButton(int index, String text, Drawable up, Drawable
		// checked, BitmapFont font) {
		...
		private static TextButtonStyle toTextButtonStyle(TabbedPaneStyle style) {
			TextButtonStyle buttonStyle = new TextButtonStyle(
					style.titleButtonUnselected, null,
					style.titleButtonSelected, style.font);
	...
	static public class TabbedPaneStyle {
		...
		public TabbedPaneStyle(Drawable titleBegin, Drawable titleEnd,
				Drawable bodyBackground, Drawable titleButtonSelected,
