...
/**
 * <p>
 * A copy of libgdx's Mesh using integer indices for vertices. This gives us the possibility to index more indices.
 * </p>
 * <p>
 * A MeshInt holds vertices composed of attributes specified by a {@link VertexAttributes} instance. The vertices are held either in
 ...
 * call. A managed MeshInt will be reloaded automagically so you don't have to do this manually.
 * </p>
 *
 * <p>
 * A MeshInt consists of vertices and optionally indices which specify which vertices define a triangle. Each vertex is composed of
 * attributes such as position, normal, color or texture coordinate. Note that not all of this attributes must be given, except
 * for position which is non-optional. Each attribute has an alias which is used when rendering a MeshInt in OpenGL ES 2.0. The alias
...
public class MeshInt implements Disposable {
    public enum VertexDataType {
        VertexArray, VertexBufferObject, VertexBufferObjectSubData, VertexBufferObjectWithVAO
    }

    /** list of all meshes **/
    static final Map<Application, Array<MeshInt>> meshes = new HashMap<Application, Array<MeshInt>>();

    final VertexData vertices;
    final IndexDataInt indices;
    boolean autoBind = true;
    final boolean isVertexArray;

    protected MeshInt (VertexData vertices, IndexDataInt indices, boolean isVertexArray) {
    ...
    /** Creates a new MeshInt with the given attributes.
     *
    ...
    public MeshInt (boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
        vertices = makeVertexBuffer(isStatic, maxVertices, new VertexAttributes(attributes));
        indices = new IndexBufferObjectInt(isStatic, maxIndices);
        isVertexArray = false;
     ...
     *           normal or texture coordinate */
    public MeshInt (boolean isStatic, int maxVertices, int maxIndices, VertexAttributes attributes) {
    ...
    /** by jw: Creates a new MeshInt with the given attributes. Adds extra optimizations for dynamic (frequently modified) meshes.
     *
    ...
    public MeshInt (boolean staticVertices, boolean staticIndices, int maxVertices, int maxIndices, VertexAttributes attributes) {
        vertices = makeVertexBuffer(staticVertices, maxVertices, attributes);
        indices = new IndexBufferObjectInt(staticIndices, maxIndices);
        isVertexArray = false;
    ...
    /** Creates a new MeshInt with the given attributes. This is an expert method with no error checking. Use at your own risk.
     *
     ...
     * @param attributes the {@link VertexAttribute}s. Each vertex attribute defines one property of a vertex such as position,
     *           normal or texture coordinate */
    public MeshInt (VertexDataType type, boolean isStatic, int maxVertices, int maxIndices, VertexAttribute... attributes) {
        switch(type) {
        ...
        case VertexBufferObjectSubData:
            ...
            indices = new IndexBufferObjectSubDataInt(isStatic, maxIndices);
            isVertexArray = false;
            break;
        ...
        default:
            ...
            indices = new IndexArrayInt(maxIndices);
            isVertexArray = true;
            break;
        }
    ...
    /** Sets the vertices of this MeshInt. The attributes are assumed to be given in float format.
     *
     * @param vertices the vertices.
     * @return the mesh for invocation chaining. */
    public MeshInt setVertices (float[] vertices) {
     ...
     * @return the mesh for invocation chaining. */
    public MeshInt setVertices (float[] vertices, int offset, int count) {
    ...
    public MeshInt updateVertices (int targetOffset, float[] source) {
    ...
    public MeshInt updateVertices (int targetOffset, float[] source, int sourceOffset, int count) {
        this.vertices.updateVertices(targetOffset, source, sourceOffset, count);
        return this;
    }

    /** Copies the vertices from the MeshInt to the float array. The float array must be large enough to hold all the MeshInt's vertices.
     * @param vertices the array to copy the vertices to */
    public float[] getVertices (float[] vertices) {
        return getVertices(0, -1, vertices);
    }

    /** Copies the the remaining vertices from the MeshInt to the float array. The float array must be large enough to hold the
     * remaining vertices.
    ...
    /** Copies the specified vertices from the MeshInt to the float array. The float array must be large enough to hold count vertices.
     * @param srcOffset the offset (in number of floats) of the vertices in the mesh to copy
    ...
    /** Copies the specified vertices from the MeshInt to the float array. The float array must be large enough to hold
    ...
    /** Sets the indices of this MeshInt
     *
     * @param indices the indices
     * @return the mesh for invocation chaining. */
    public MeshInt setIndices (int[] indices) {
     ...
     * @return the mesh for invocation chaining. */
    public MeshInt setIndices (int[] indices, int offset, int count) {
        this.indices.setIndices(indices, offset, count);

        return this;
    }

    /** Copies the indices from the MeshInt to the int array. The int array must be large enough to hold all the MeshInt's indices.
     * @param indices the array to copy the indices to */
    public void getIndices (int[] indices) {
        getIndices(indices, 0);
    }

    /** Copies the indices from the MeshInt to the int array. The int array must be large enough to hold destOffset + all the
     * MeshInt's indices.
     * @param indices the array to copy the indices to
    ...
    /** Copies the remaining indices from the MeshInt to the int array. The int array must be large enough to hold destOffset + all
    ...
    /** Copies the indices from the MeshInt to the int array. The int array must be large enough to hold destOffset + count
     * indices.
     * @param srcOffset the zero-based offset of the first index to fetch
     ...
     * MSM720xa chips that will fuck up memory if you manipulate the vertices and indices of a MeshInt multiple times while it is
    ...
    /** Binds the underlying {@link VertexBufferObject} and {@link IndexBufferObjectInt} if indices where given. Use this with OpenGL
    ...
    /** Unbinds the underlying {@link VertexBufferObject} and {@link IndexBufferObjectInt} is indices were given. Use this with OpenGL
     ...
     * @param offset the offset into the vertex or index buffer
     * @param count number of vertices or indices to use
     * @param autoBind overrides the autoBind member of this MeshInt */
    ...
    /** Frees all resources associated with this MeshInt */
    public void dispose () {
    ...
    /** @return the vertex attributes of this MeshInt */
    ...
    private static void addManagedMesh (Application app, MeshInt mesh) {
        Array<MeshInt> managedResources = meshes.get(app);
        if (managedResources == null) managedResources = new Array<MeshInt>();
    ...
    public static void invalidateAllMeshes (Application app) {
        Array<MeshInt> meshesArray = meshes.get(app);
    ...
    public MeshInt copy (boolean isStatic, boolean removeDuplicates, final int[] usage) {
        ...
        MeshInt result;
        if (attrs == null)
            result = new MeshInt(isStatic, numVertices, indices == null ? 0 : indices.length, getVertexAttributes());
        else
            result = new MeshInt(isStatic, numVertices, indices == null ? 0 : indices.length, attrs);
     ...
     * @param isStatic whether the new mesh is static or not. Allows for internal optimizations.
     * @return the copy of this mesh */
    public MeshInt copy (boolean isStatic) {
