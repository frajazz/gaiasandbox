 ...
 *
 */
public class OrbitFileDataProviderESA implements IOrbitDataProvider {
    OrbitData data;

    @Override
    public void load(String file, OrbitDataLoaderParameter parameter) {
        FileDataLoaderESA odl = new FileDataLoaderESA();
        try {
            FileHandle f = Gdx.files.internal(file);
            data = odl.load(f.read());
            EventManager.instance.post(Events.ORBIT_DATA_LOADED, data, file);
        } catch (Exception e) {
            Gdx.app.error(OrbitFileDataProviderESA.class.getName(), e.getMessage());
        }
    }
